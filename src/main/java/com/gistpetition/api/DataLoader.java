...
public class DataLoader implements CommandLineRunner {
    ...
    public void loadData() {
        ...
        User a = userRepository.save(new User("a@gist.ac.kr", PASSWORD, UserRole.USER));
        User b = userRepository.save(new User("b@gist.ac.kr", PASSWORD, UserRole.USER));
        User c = userRepository.save(new User("c@gist.ac.kr", PASSWORD, UserRole.USER));
        User d = userRepository.save(new User("d@gist.ac.kr", PASSWORD, UserRole.USER));
        User e = userRepository.save(new User("e@gist.ac.kr", PASSWORD, UserRole.USER));
        List<User> users = Arrays.asList(a, b, c, d, e);

        Petition petition1 = petitionRepository.save(new Petition("국민 청원에 글을 써버렸다.", CONTENT, Category.DORMITORY, a.getId()));
        Petition petition2 = petitionRepository.save(new Petition("방송 촬영을 위해 안전과 생존을 위협당하는 동물의 대책 마련이 필요합니다", CONTENT, Category.DORMITORY, a.getId()));
        Petition petition3 = petitionRepository.save(new Petition("길고양이를 학대하는 갤러리를 폐쇄하고 엄중한 수사를 해주십시오.", CONTENT, Category.DORMITORY, a.getId()));
        Petition petition4 = petitionRepository.save(new Petition("코로나19로 부터 우리 아이들을 지켜주세요.", CONTENT, Category.DORMITORY, a.getId()));
        Petition petition5 = petitionRepository.save(new Petition("2차방역지원금 지금 자영업분들 농락하시나요?", CONTENT, Category.DORMITORY, a.getId()));
